...
function AmbassadorsOfCultureAndArts({ navigation }) {
  ...
  const handleOptionChange = (option) => {
    ...
    const correctAnswerIndex = questions4[currentQuestion].answer;
    const selectedOptionIndex = questions4[currentQuestion].options.indexOf(option);
  ...
  const handleNextQuestion = () => {
    ...
    setSelectedOption(0);
    setIsCorrect(false);
    if (currentQuestion < questions4.length - 1) {
        ...
        <View>
          <View style={{ alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 20, marginVertical: 10, textAlign: 'justify', marginHorizontal: 10 }}>
              {questions4[currentQuestion].question}
            </Text>
          </View>
          <View style={{ alignItems: 'center' }}>
            {questions4[currentQuestion].options.map((option, index) => (
              <TouchableOpacity
                key={index}
                style={{
          ...
          {showAnswer && (
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={{ fontSize: 16 }}>The correct answer is: {questions4[currentQuestion].options[questions4[currentQuestion].answer]}</Text>
              <TouchableOpacity
                style={{
                  backgroundColor: '#3498db',
                  padding: 10,
  ...
  export default AmbassadorsOfCultureAndArts;